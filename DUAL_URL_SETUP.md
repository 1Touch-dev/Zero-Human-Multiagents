...
- Existing deployment stays live on `http://<server-ip>:3000`
- New Multiagents deployment runs on `http://<server-ip>:3101`
...
Without this step, `localhost:3101` may work on the server but `http://<server-ip>:3101` will not open from your browser.

## 3) Verify both URLs

- Existing URL: `http://<server-ip>:3000/PAP/dashboard`
- New URL: `http://<server-ip>:3101`
...
Then open inbound TCP `3200` and browse `http://<server-ip>:3200`.
